...
export default function ManageWorkersScreen() {
    ...
    const fetchWorkers = async () => {
        ...
        // 1. Get the profile to figure out WHO is looking at this screen
        const { data: profile } = await supabase.from('profiles').select('*').eq('id', user.id).single();

        // 2. Save their role so the UI knows whether to hide the Edit buttons
        setUserRole(profile?.role || 'worker');

        // 3. Figure out the Workshop ID safely
        const myWorkshopId = profile?.role === 'admin' ? profile.id : profile?.admin_id;

        // 4. Fetch the roster for this workshop
                        ...
                        workers.map((worker, index) => (
                            <View key={worker.id}>
                                <TouchableOpacity
                                    ...
                                    activeOpacity={userRole === 'admin' ? 0.7 : 1} // 🚀 Disables click flash for workers
                                    onPress={() => userRole === 'admin' && openEditModal(worker)} // 🚀 Only admins can click to edit
                                >
                                    ...
                                    {/* 🚀 ONLY SHOW THE EDIT BADGE IF ADMIN */}
                                    {userRole === 'admin' && (
...
const styles = StyleSheet.create({
    ...
    modalOverlay: { flex: 1, backgroundColor: 'rgba(0, 0, 0, 0.75)', padding: 20 },
